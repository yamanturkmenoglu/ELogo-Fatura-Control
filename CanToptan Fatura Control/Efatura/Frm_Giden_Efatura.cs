...
{
    public partial class Frm_Giden_Efatura : Form
    {
        public Frm_Giden_Efatura()
        {
            InitializeComponent();
        ...
        private void Frm_Giden_Efatura_Load(object sender, EventArgs e)
        {
            FillType();
        ...
        private DataTable ParseSoapResponse(string soapResponse)
        {
            ...
            foreach (var document in documents)
            {
                ...
                if (docInfoElement != null)
                {
                    ...
                    row["Fatura Tipi"] = jsonObj["InvoiceType"]?.ToString();
                    ...
                    row["Vergi Toplamı"] = jsonObj["TaxAmount"]?.ToString();
                    row["Toplam Tutar"] = jsonObj["TaxInclusiveAmount"]?.ToString();
                    row["ETTN"] = jsonObj["Uuid"]?.ToString();
                    row["Ödenecek Tutar"] = jsonObj["PayableAmount"]?.ToString();
